...
- One new API endpoint: `POST /api/notes/[id]/move` (atomic move from bin A to bin B)
- One new API endpoint: `GET /api/bins/[id]/preview-delete` (counts for confirmation)
- One bug fix in `mergeBin`: re-parent source bin's children to target (currently they would cascade-delete, which is incorrect for a merge)
- Cycle-detection helper for move/re-parent (reject if target is self or descendant)
- Tests: extended `tests/lib/queries/bins.test.ts` + new `tests/app/api/bins/` directory
...
- Pattern lifted from existing `components/QuickCapture.tsx`
...
// Response 200:
{
  ...
  note_count: number                 // notes assigned only to this bin or its descendants
}
```
...
**None.** The existing `bins` and `note_bins` tables support every operation in this spec. Cycle detection is enforced in code, not schema.
...
Reused for: New child bin (parent picker, only when launched from "+ new bin"), Move bin…, Merge into…, Add note to bin…, Move note to bin….
...
**Sort order strategy:** client-side averaging. When dropped between two siblings with sort_orders A and B, the new value is `(A + B) / 2`. When dropped at the start, `first_sibling.sort_order - 1000`. When dropped at the end, `last_sibling.sort_order + 1000`. Initial values are `0, 1000, 2000, …`. This avoids server-side renumbering and supports many reorders before precision becomes an issue (well past the user's expected scale). Use REAL/float `sort_order` if needed — current schema is INTEGER but accepts large values; switch to REAL only if precision becomes a problem.
...
| Note row | Sidebar bin row | ⌘ held | **Move** | `POST /api/notes/[id]/move` (or assign + skip if no source bin context) |
...
- Note count is **direct assignments to source bin only** — sub-bins are re-parented (not merged), so their notes stay assigned to the sub-bins, which keep their identity. Counts come from `GET /api/bins/[id]/preview-delete` (reuse the same endpoint; sub-bin re-parent is a separate concern visible in the warning line)
...
Test harness: same `resetDbForTesting()` pattern as queries, plus a thin wrapper to invoke route handlers directly:

```ts
import { POST } from "@/app/api/notes/[id]/move/route";
const res = await POST(new Request("http://x", { method: "POST", body: JSON.stringify({ from_bin_id, to_bin_id }) }), { params: { id: noteId } });
```
...
- `components/Modal.tsx` — generic overlay
...
- `app/api/bins/[id]/route.ts` — add cycle validation in PATCH path with `parent_bin_id`; PATCH with `merge_into` triggers updated `mergeBin`
- `lib/queries/bins.ts` — add `getBinDeletePreview`, `isDescendantOf`, `moveNoteBetweenBins`; update `mergeBin` to re-parent children
...
| Multi-bin model | Allowed; drag = Add, ⌘+drag = Move |
| Seeded bins (`notion-sync`) | Rename allowed, Delete hidden — revisit at v1.4 |
